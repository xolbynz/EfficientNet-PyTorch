...
def predict(dataset):
    ...
    pretrain_dict = torch.load("/works/model_best_eff.pth", map_location=device)
...
def main():
    ...
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=32, num_workers=16, shuffle=False)    
    ...
    submission.to_csv("/works/EfficientNet-PyTorch/examples/imagenet/data/baseline_submission_efficientnetv2_rw_m_90.csv", index=False)
